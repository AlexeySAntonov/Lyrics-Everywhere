...
class SearchFragment : BaseFragment(), SearchFragmentView {
  ...
  override fun showData(data: List<BaseData>, query: String) {
    ...
    if (adapter == null) {
      adapter = DataAdapter(
          data,
          DataAdapter.OnTrackClickListener { this.launchTrackDetailsActivity(it) },
          DataAdapter.OnTwitterClickListener { this.launchTwitter(it) },
          MyApplication.imageRequestManager,
          query)
